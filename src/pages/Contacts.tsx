...
import { ArrowLeftIcon, QrCodeIcon, UserPlusIcon, TrashIcon } from "@heroicons/react/24/outline";
...
export const Contacts: React.FC = () => {
  ...
  const { contacts, addOrUpdateContact, removeContact } = useContacts();
  ...
  const canAddManual = useMemo(() => {
    return manualName.trim().length > 0 && isLikelyAddress(manualAddress.trim());
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-5 sm:p-6 shadow-lg border border-white/20">
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-center">
                  <p className="font-mono text-sm text-gray-700 break-all">{myAddress}</p>
                </div>
              </div>
            </div>

            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-5 sm:p-6 shadow-lg border border-white/20">
              <h2 className="text-lg font-semibold text-gray-900 mb-3">Scan To Add</h2>
              <p className="text-sm text-gray-600 mb-4">
                Ask your friend to open this Contacts page and show their QR. Scan it to save their address with a name.
              </p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-xs text-gray-600">
                        {scannerReady ? "Point your camera at a QR code" : "Starting camera..."}
                      </div>
                      <button
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-5 sm:p-6 shadow-lg border border-white/20">
              <h2 className="text-lg font-semibold text-gray-900 mb-3">Add Manually</h2>
              <div className="grid grid-cols-1 gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    addOrUpdateContact({ name: manualName.trim(), address: manualAddress.trim() });
                    setManualName("");
                    setManualAddress("");
                  }}
          ...
          <div className="lg:col-span-7">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-5 sm:p-6 shadow-lg border border-white/20">
              <h2 className="text-lg font-semibold text-gray-900 mb-1">Saved Contacts</h2>
              <p className="text-xs text-gray-500 mb-3">Contacts are stored locally in your browser using localStorage.</p>
              {contacts.length === 0 ? (
                <div className="text-center py-10 text-gray-600">No contacts saved yet.</div>
              ) : (
                <div className="divide-y divide-gray-100">
                  {contacts.map((c) => (
                    <div key={c.address} className="flex items-center justify-between py-3">
                      <div>
...
function parseScannedPayload(payload: string): string | null {
  ...
  try {
    ...
    if (obj && typeof obj.address === "string" && isLikelyAddress(obj.address)) {
      return obj.address;
    }
  } catch {}
  return null;
...
function suggestNameFromAddress(address: string, all: { name: string; address: string }[]) {
